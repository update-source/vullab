...
const {
    User, UserProfile, Category, Product, ProductItem,
    OrderStatus, ShippingMethod, PaymentMethod, Country
} = require('../models');
...
const seedDatabase = async () => {
    ...
    try {
        ...
        await seedReferenceTable(OrderStatus, referenceData.orderStatuses, ['code'], t);
        await seedReferenceTable(PaymentMethod, referenceData.paymentMethods, ['code'], t);
        await seedReferenceTable(ShippingMethod, referenceData.shippingMethods, ['code'], t);
        await seedReferenceTable(Country, referenceData.countries, ['countryCode'], t);
        ...
        if (isDev) {
            ...
            const phoneCat = await Category.findOne({ where: { slug: 'dien-thoai' }, transaction: t });
            const laptopCat = await Category.findOne({ where: { slug: 'laptop' }, transaction: t });

            if (phoneCat && laptopCat) {
                for (const prodData of dummyData.products) {
                    // Xác định category dựa trên logic đơn giản (demo)
                    const catId = prodData.name.includes('iPhone') ? phoneCat.id : laptopCat.id;
